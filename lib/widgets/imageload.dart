...
class CustomNetworkImage extends StatefulWidget {
  ...
  final int? currentPhotoIndex; // Make currentPhotoIndex optional
  final Function(int)? onImageLoaded; // Make onImageLoaded optional

  const CustomNetworkImage({
    Key? key,
    required this.imageUrl,
    this.index, // Optional
    this.currentPhotoIndex, // Optional
    this.onImageLoaded, // Optional
  }) : super(key: key);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          // If onImageLoaded is provided, call it
          if (widget.index != null && widget.onImageLoaded != null) {
            if (widget.index == widget.currentPhotoIndex) {
              widget.onImageLoaded!(widget.index!); // Trigger callback
            }
          }
        });
      } else {
